...
@task
def load_and_preprocess_pm_data(file_path, year=2013, chunk_size=111, max_lags=10):
    """Load PM data and process Lat, Lon, elevation..."""
    data = pl.read_csv(file_path)
    total_rows = data.shape[0]
    pm_data = pl.DataFrame()
    if year is not None:
        data = select_year(data, year, date_key="Date")
        pm_data = load_and_preprocess_pm_data_chunk(data, max_lags, 0, total_rows)
    else: 
...
@task
def load_and_preprocess_gridmet_data(file_path, year):
    """Load and preprocess GridMET data."""
    print("Loading GridMET data...")
    data = pl.read_csv(file_path)
    if year is not None:
        data = select_year(data, year, date_key="pm_Date")
...
@task
def select_year(data, year, date_key):
    """Normalize specified columns in the dataframe."""
    data = data.with_columns(pl.col(date_key).str.strptime(pl.Date,"%Y-%m-%d").dt.year().alias("year"))
...
@flow
def process_and_save_data(data_dir, year, save_name="processed_pm25_gridmet.parquet", max_lags=20, chunk_size=100000):
    ...
    pm_data = load_and_preprocess_pm_data(pm_file_path, year=year, chunk_size=chunk_size, max_lags=max_lags)
    gridmet_data = load_and_preprocess_gridmet_data(gridmet_file_path, year=year)
...
if __name__ == "__main__":
    ...
    if args.year == 0:
        args.year = None
    process_and_save_data(data_dir=args.data_dir, year = args.year, save_name=f"processed_pm25_gridmet_{args.year}.parquet" )
